...
const VideoUploader = ({ onAnalysisComplete }: VideoUploaderProps) => {
  ...
  useEffect(() => {
    const checkApiConfig = async () => {
      try {
        const { data, error } = await fetch('/api/check-api-config').then(res => res.json());
        if (error || !data?.keysConfigured) {
          setApiError('The application is not fully configured yet. Please contact the administrator.');
        }
      } catch (err) {
        console.error("Error checking API configuration:", err);
        // Don't set error here as the endpoint might not exist yet
      }
    };
  ...
  useEffect(() => {
    ...
    if (success === 'true') {
      ...
      toast({
        ...
        description: tierMessages[subscriptionTier as keyof typeof tierMessages] || "Your subscription has been activated. You can now view your analysis.",
        variant: "default",
      });
  ...
  return (
    <div className="space-y-8">
      {apiError && (
        <Alert className="bg-yellow-500/10 p-4 border-yellow-500/50">
          <AlertDescription className="text-yellow-500">{apiError}</AlertDescription>
        </Alert>
      )}
      ...
      <div className="relative">
        <label 
          className={`block p-8 rounded-lg border-2 border-dashed border-white/20 bg-black/20 cursor-pointer transition-all hover:bg-black/40 ${
            isProcessing || apiError ? 'opacity-50 cursor-not-allowed' : ''
          }`}
        >
          <input
            type="file"
            ...
            className="hidden"
            disabled={isProcessing || !!apiError}
          />
